...
public class ScheduleService implements Command {
   ...
   public String execute(HttpServletRequest request, HttpServletResponse response)
      ...
      String memId = mdto.getMemId();
		...
		/*
		 * //String type 으로 받아온 date 정보를 localdate로 형변환 DateTimeFormatter formatter =
		 * DateTimeFormatter.ofPattern("yyyy-MM-dd", Locale.US); LocalDate scheStartDt3
		 * = LocalDate.parse(scheStartDt2, formatter); LocalDate scheEndDt3 =
		 * LocalDate.parse(scheEndDt2, formatter);
		 * 
		 * // localDate -> date Date scheStartDt = java.sql.Date.valueOf(scheStartDt3);
